...
class IrradiacaoGlobal(db.Model):
  ...
  @classmethod
  def buscar(cls,cidade,estado):
    return db.session.query(IrradiacaoGlobal).filter(
      IrradiacaoGlobal.IG_CIDADE==cidade,
      IrradiacaoGlobal.IG_UF==estado
      ).order_by(IrradiacaoGlobal.IG_ID)
  def save(self):
    db.session.add(self)
    db.session.commit()
  def update(self):
